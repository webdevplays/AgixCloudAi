...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (!vantaEffect && vantaRef.current && (window as any).VANTA) {
      setVantaEffect((window as any).VANTA.HALO({
        el: vantaRef.current,
        mouseControls: true,
        touchControls: true,
        gyroControls: false,
  ...
  useEffect(() => {
    if (!visionEffect && visionVantaRef.current && (window as any).VANTA) {
      setVisionEffect((window as any).VANTA.GLOBE({
  ...
  return (
    <div className="relative min-h-screen text-white selection:bg-[#4fb7b3] selection:text-black cursor-auto md:cursor-none overflow-x-hidden">
      ...
      <AnimatePresence>
        {showMaintenance && (
          <motion.div 
            ...
            className="fixed top-24 left-1/2 translate-x-[-50%] z-[100] w-[90%] max-w-sm"
          >
            <div className="bg-white/10 backdrop-blur-xl border border-[#a8fbd3]/30 px-6 py-4 rounded-full shadow-[0_0_30px_rgba(168,251,211,0.2)] flex items-center justify-center gap-3">
              <AlertTriangle className="text-[#a8fbd3] w-5 h-5 flex-shrink-0" />
              <span className="font-heading font-bold uppercase tracking-widest text-[10px] sm:text-xs text-white text-center">Builder is undermaintenance</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            ...
            <div className="flex flex-col items-center gap-6 mt-8 sm:mt-12 w-full max-w-xs">
              <button 
                onClick={() => {
                  setMobileMenuOpen(false);
                  handleLaunchBuilder();
                }}
                className="w-full bg-white text-black py-4 font-heading font-black tracking-widest uppercase text-sm"
              >
                Launch Builder
              </button>
              <div className="flex items-center gap-8 text-white/50">
                <a href="https://t.me/AgixCloudAI" target="_blank" rel="noopener noreferrer"><Send className="w-6 h-6" /></a>
                <a href="https://x.com/AgixCloud" target="_blank" rel="noopener noreferrer"><Twitter className="w-6 h-6" /></a>
                <a href="https://github.com/agixcloud/Agix-cloud-Ai-Builder" target="_blank" rel="noopener noreferrer"><Github className="w-6 h-6" /></a>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <header className="relative h-[100svh] min-h-[600px] flex flex-col items-center justify-center overflow-hidden px-4 md:px-8">
        <div ref={vantaRef} className="absolute inset-0 z-0 opacity-60 pointer-events-none" />
