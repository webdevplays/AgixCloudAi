...
const StarField: React.FC<StarFieldProps> = ({ parallaxY }) => {
...
const FluidBackground: React.FC = () => {
  ...
  // Content moves ~100% of height, background layers move much less
  const starsY = useTransform(scrollYProgress, [0, 1], [0, -80]);
  const blobsY = useTransform(scrollYProgress, [0, 1], [0, -200]);

  return (
    <div className="fixed inset-0 -z-10 overflow-hidden bg-gradient-to-br from-[#31326f] via-[#28295c] to-[#1f2048]">
      
      <StarField parallaxY={starsY} />

      {/* Layer for parallax blobs */}
